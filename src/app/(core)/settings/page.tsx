    ...
    <div className="flex flex-col h-full">
      <main className="flex-1 p-6 md:p-8 max-w-[1440px] mx-auto w-full space-y-8">
        ...
        <div className="max-w-2xl space-y-8">
          ...
          <div>
            <h2 className="text-xs uppercase tracking-widest text-zinc-500 font-bold mb-4 flex items-center gap-2">
              <Key className="w-3.5 h-3.5" /> External Integrations
            </h2>
            <div className="glass-card p-5 space-y-4">
              <p className="text-[11px] text-zinc-600 mb-3">Integration status is managed via environment configuration. Contact your system administrator to add or rotate credentials.</p>
              {[
                { label: "AI Intelligence Engine", service: "Google Gemini", status: "connected" },
                { label: "Authentication Provider", service: "Clerk", status: "connected" },
                { label: "Real-time Database", service: "Convex", status: "connected" },
                { label: "Video AI Layer", service: "Avatar Service", status: "not_configured" },
              ].map((integration) => (
                <div key={integration.label} className="flex items-center justify-between py-2 border-b border-white/5 last:border-0">
                  <div>
                    <p className="text-xs font-medium text-white">{integration.label}</p>
                    <p className="text-[10px] text-zinc-600 mt-0.5">{integration.service}</p>
                  </div>
