...
import { Search, ChevronDown, ChevronRight, Activity, Globe, ShieldCheck, Zap, FileText, Users, FileBarChart, CheckCircle2, Circle, Lock, BrainCircuit } from "lucide-react";
...
export default function SEOAuditPage() {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (data.passed) {
        setPassedChecks(data.passed);
        alert(`Scan Complete: ${data.passed.length} markers identified for ${data.url}`);
      } else {
        alert(data.error || "Scan failed to return valid data.");
      }
    } catch (error) {
      console.error("Scan Error:", error);
      alert("Critical Connection Error during scan.");
    } finally {
      setIsScanning(false);
    }
  };
    ...
    <div className="flex h-full w-full bg-black text-zinc-300 font-sans p-6 overflow-y-auto">
      <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="col-span-1 lg:col-span-2 space-y-6">
          <div>
            <h1 className="text-3xl font-bold text-white tracking-tight mb-2">SGDIO Observatory</h1>
            <p className="text-zinc-400">Sovereign 260-Point Institutional Digital Governance Framework for Somaliland MDAs.</p>
          </div>
          ...
          <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4 flex gap-4">
            ...
            <Button 
              onClick={handleScan}
              disabled={isScanning}
              className="bg-[#D4AF37] hover:bg-[#B3932F] text-black font-semibold"
            >
              <Search className="w-4 h-4 mr-2" /> 
              {isScanning ? "Scanning Intelligence..." : "Trigger Intelligence Scan"}
            </Button>
          </div>
            ...
            <div className="flex flex-col items-center justify-center mb-8">
              <div className="relative w-48 h-48 flex items-center justify-center rounded-full border-[12px] border-zinc-800">
                ...
                <div className="absolute inset-[2px] bg-black rounded-full flex flex-col items-center justify-center">
                  ...
                  <span className="text-[10px] text-zinc-600 mt-1 uppercase tracking-tighter">Weighted Governance Score</span>
                </div>
              </div>
